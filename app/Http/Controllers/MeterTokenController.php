...
class MeterTokenController extends Controller
{
    ...
    public function clearTamperRecord(ClearMeterTokenRequest $request, PrepaidMeterService $prepaidMeterService): JsonResponse
    {
        $meter = Meter::find($request->meter_id);
        try {
            $token = $prepaidMeterService->clearTamperRecord($meter->number, $meter->category, $meter->prepaid_meter_type, $meter->use_prism_vend);
            throw_if($token === null || $token === '', RuntimeException::class, 'Failed to clear credit token');
            $token = strtok($token, ',');
    ...
    public function show(string $id): JsonResponse
    {
        ...
        if ($meter_token->user->should_pay_connection_fee){
            $meter_token->user->connection_fee_balance = $this->getUserConnectionFeeBalance($meter_token->user);
        }
